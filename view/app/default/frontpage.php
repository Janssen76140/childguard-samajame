<section id="section1Homepage" style="background-image: url('../public/asset/img/bansamajamegrosse.svg')">
    <div class="opacity">
        <div class="titleImage">

        </div>
        <div class="rechercheCreche">

            <p class="pres1">Trouver une place ?<br>Un jeu d'enfant !</p>
            <form action="" method="get">
                <div>
                    <input id="adresse_search" name="distance" value="" placeholder="Indiquez votre adresse"
                           type="text">
                </div>
                <div>
                    <select name="distance" id="distance"></select>
                </div>
                <div>
                    <input id="go" type="submit" name="submitted" value="GO !">
                </div>
            </form>
        </div>

        <div class="professionnel">
            <p class="pres2">Espace Pro</p>
            <input id="pro" type="submit" name="submitted" value="C'est par ici !">
        </div>


    </div>
    <div class="clear"></div>
</section>

<section id="section2Homepage">
    <h3 class="service">Nos <span class="autreCouleur">services:</span></h3>
    <div class="nosServices">
        <div class="service1">
            <div class="bleu">
                <div class="imgService">
                    <img class="icone icone1" src="asset/img/creche.png" alt=""/>
                </div>
                <div class="divParaService">
                    <p class="paraService paraService1">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Alias expedita explicabo facere impedit ipsum maxime modi molestiae odio optio provident, quam qui quia quo quos saepe, suscipit tenetur totam. Quos!</p>
                </div>
                <div class="clear"></div>
            </div>
            <div class="rouge">
                <div class="imgService">
                    <img class="icone" src="asset/img/geoloc.png" alt=""/>
                </div>
                <div class="divParaService">
                    <p class="paraService paraService2">Lorem ipsum dolor sit amet, consectetur adipisicing elit.
                        Cupiditate hic impedit incidunt iste laudantium minus optio possimus quia repellendus voluptatibus! Ad adipisci deleniti error esse numquam quod reiciendis reprehenderit vitae!</p>
                </div>
                <div class="clear"></div>
            </div>
        </div>

        <div class="service2">
            <div class="orange">
                <div class="imgService">
                    <img class="icone" src="asset/img/geoloc.png" alt=""/>
                </div>
                <div class="divParaService">
                    <p class="paraService paraService3">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Ab aperiam assumenda at dolorum eveniet fugit ipsa minus numquam omnis possimus quia, quibusdam ratione rem, sed totam! Ab cupiditate nostrum sunt.</p>
                </div>
                <div class="clear"></div>
            </div>
            <div class="vert">
                <div class="imgService">
                    <img class="icone" src="asset/img/geoloc.png" alt=""/>
                </div>
                <div class="divParaService">
                    <p class="paraService paraService4">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Accusamus accusantium animi aspernatur culpa debitis deleniti doloribus esse est illo illum laboriosam nemo quae, quod recusandae reprehenderit veniam voluptate voluptatem voluptatum?</p>
                </div>
                <div class="clear"></div>
            </div>
        </div>
    </div>
</section>
<div class="clear"></div>
